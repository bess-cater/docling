                ...
                with TimeRecorder(conv_res, "table_structure"):
                    ...
                    if len(table_bboxes):
                        for table_cluster, tbl_box in in_tables:
                            ...
                            # Move table predictions to CPU to free GPU memory
                            try:
                                import torch
                                if torch.cuda.is_available():
                                    # Move any GPU tensors in tf_output to CPU
                                    for i, output in enumerate(tf_output):
                                        if isinstance(output, dict):
                                            for key, value in output.items():
                                                if hasattr(value, 'cpu'):
                                                    try:
                                                        tf_output[i][key] = value.cpu()
                                                    except:
                                                        pass
                                        elif hasattr(output, 'cpu'):
                                            try:
                                                tf_output[i] = output.cpu()
                                            except:
                                                pass
