...
class PaddleOcrModel(BaseOcrModel):
    def __init__(
        ...
        #     cpu_model = TFSMLayer(model_path, call_endpoint="serving_default")
            ...
            if not page._backend.is_valid():
                ...
                with TimeRecorder(conv_res, "ocr"):
                    ...
                    for ocr_rect_i, ocr_rect in enumerate(ocr_rects):
                        ...
                        # Move PaddleOCR results to CPU to free GPU memory
                        try:
                            import torch
                            if torch.cuda.is_available():
                                # Move any GPU tensors in result to CPU
                                for i, res_item in enumerate(result):
                                    if isinstance(res_item, dict):
                                        for key, value in res_item.items():
                                            if hasattr(value, 'cpu'):
                                                try:
                                                    result[i][key] = value.cpu()
                                                except:
                                                    pass
                                    elif hasattr(res_item, 'cpu'):
                                        try:
                                            result[i] = res_item.cpu()
                                        except:
                                            pass
                                
                                # CRITICAL: Clear PaddlePaddle GPU memory
                                try:
                                    import paddle
                ...
                try:
                    ...
                    if hasattr(paddle.device, 'cuda') and hasattr(paddle.device.cuda, 'empty_cache'):
                        paddle.device.cuda.empty_cache()
                        print(f"🧹 [PADDLE CLEAR] Cleared PaddlePaddle GPU cache after page")
                except Exception as pp_error:
                    print(f"⚠️ Could not clear PaddlePaddle cache: {pp_error}")
